...
def extract_budget_data(pdf_path: str) -> dict:
    ...
    system_prompt = (
        ...
        "A SEPARATE APPROPRIATION has its own section title and its own distinct dollar figure "
        "in the bill — it stands alone as a named recipient. Include it regardless of size.\n"
        "  Example: 'Governor's Office — Custodial Fund: $653,000' is its own section → include it.\n"
        "A SUB-ALLOCATION is a line item appearing INSIDE another entity's section that shows "
        "how that entity's already-stated total is broken down among internal programs or purposes. "
        "  Example: within the 'State Dept of Education — $3.1B' section, lines for Reading "
        "Initiative $151M and Numeracy Act $114M → these are breakdowns of the $3.1B, exclude them.\n"
        "When in doubt: if the item has its own titled section and its own figure not already "
        "captured in a parent total, include it.\n\n"
